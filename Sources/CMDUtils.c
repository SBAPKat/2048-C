...
void Print_Cases(int row, int col, int** GameMatrix){
    ...
    for(int y=0 ;y<=3;y++){
        for(int x=0;x<=3;x++){
			if(GameMatrix[x][y]<=2048) attron(COLOR_PAIR(GameMatrix[x][y]));
            ...
            refresh();
			if(GameMatrix[x][y]<=2048) attroff(COLOR_PAIR(GameMatrix[x][y]));
        }
...
void Def_Colors(){
	...
	init_pair(128,COLOR_RED,COLOR_BLACK);			//128
